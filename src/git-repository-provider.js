...
const pathFromGitFile = function(gitFile) {
  try {
...
var findGitDirectorySync = function(directory) {
  ...
  const gitDirPath = pathFromGitFile(typeof gitDir.getPath === 'function' ? gitDir.getPath() : undefined);
  if (gitDirPath) {
    gitDir = new Directory(directory.resolve(gitDirPath));
  }
  if ((typeof gitDir.existsSync === 'function' ? gitDir.existsSync() : undefined) && isValidGitDirectorySync(gitDir)) {
    return gitDir;
...
var isValidGitDirectorySync = directory =>
...
module.exports =
(GitRepositoryProvider = class GitRepositoryProvider {
