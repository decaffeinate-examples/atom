...
describe('TokenizedBuffer', function () {
  ...
  describe('when the buffer contains soft-tabs', function () {
    ...
    describe('on construction', () =>
      it('tokenizes lines chunk at a time in the background', function () {
    ...
    describe('when the buffer is partially tokenized', function () {
      ...
      describe('when there is a buffer change inside the tokenized region', function () {
        describe('when lines are added', () =>
          it('pushes the invalid rows down', function () {
        ...
        describe('when lines are removed', () =>
          it('pulls the invalid rows up', function () {
            expect(tokenizedBuffer.firstInvalidRow()).toBe(5)
        ...
        return describe('when the change invalidates all the lines before the current invalid region', () =>
          it('retokenizes the invalidated lines and continues into the valid region', function () {
            expect(tokenizedBuffer.firstInvalidRow()).toBe(5)
            buffer.insert([2, 0], '/*')
      ...
      describe('when there is a buffer change surrounding an invalid row', () =>
        it('pushes the invalid row to the end of the change', function () {
          buffer.setTextInRange([[4, 0], [6, 0]], '\n\n\n')
          return expect(tokenizedBuffer.firstInvalidRow()).toBe(8)
        })
      )

      return describe('when there is a buffer change inside an invalid region', () =>
        it('does not attempt to tokenize the lines in the change, and preserves the existing invalid row', function () {
          expect(tokenizedBuffer.firstInvalidRow()).toBe(5)
    ...
    return describe('when the buffer is fully tokenized', function () {
      ...
      describe('when there is a buffer change that is smaller than the chunk size', function () {
        describe('when lines are updated, but none are added or removed', function () {
          ...
          describe('when the change invalidates the tokenization of subsequent lines', () =>
            it('schedules the invalidated lines to be tokenized in the background', function () {
              buffer.insert([5, 30], '/* */')
              buffer.insert([2, 0], '/*')
        ...
        return describe('when the change invalidates the tokenization of subsequent lines', () =>
          it('schedules the invalidated lines to be tokenized in the background', function () {
            buffer.insert([5, 30], '/* */')
            buffer.insert([2, 0], '/*\nabcde\nabcder')
            expect(tokenizedBuffer.tokenizedLines[2].tokens[0].scopes).toEqual(['source.js', 'comment.block.js', 'punctuation.definition.comment.js'])
      ...
      describe('when there is an insertion that is larger than the chunk size', () =>
        it('tokenizes the initial chunk synchronously, then tokenizes the remaining lines in the background', function () {
  ...
  describe('when the buffer contains hard-tabs', function () {
    ...
    return describe('when the buffer is fully tokenized', () =>
      beforeEach(() => fullyTokenize(tokenizedBuffer))
    )
  })
  ...
  describe('.bufferRangeForScopeAtPosition(selector, position)', function () {
    ...
    describe('when the selector matches a single token at the position', () =>
      it('returns the range covered by the token', function () {
    ...
    return describe('when the selector matches a run of multiple tokens at the position', () =>
      it('returns the range covered by all contigous tokens (within a single line)', () => expect(tokenizedBuffer.bufferRangeForScopeAtPosition('.function', [1, 18])).toEqual([[1, 6], [1, 28]]))
    )
  })
  ...
  describe('.indentLevelForRow(row)', function () {
    ...
    describe('when the line is non-empty', () =>
      it('has an indent level based on the leading whitespace on the line', function () {
    ...
    describe('when the line is empty', () =>
      it('assumes the indentation level of the first non-empty line below or above if one exists', function () {
        buffer.insert([12, 0], '    ')
  ...
  describe('when the buffer is configured with the null grammar', () =>
    it('does not actually tokenize using the grammar', function () {
  ...
  return describe('text decoration layer API', () =>
