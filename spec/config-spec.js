...
describe('Config', function () {
  ...
  describe('.get(keyPath, {scope, sources, excludeSources})', function () {
    ...
    return describe("when a 'scope' option is given", function () {
      ...
      return describe('when package settings are added after user settings', () =>
  ...
  describe('.set(keyPath, value, {source, scopeSelector})', function () {
    ...
    describe('when the key-path is null', () =>
      it('sets the root object', function () {
    ...
    describe('when the value equals the default value', () =>
      it("does not store the value in the user's config", function () {
    ...
    return describe("when a 'scopeSelector' is given", () =>
      it('sets the value and overrides the others', function () {
        atom.config.set('foo.bar.baz', 42, { scopeSelector: '.source.coffee .string.quoted.double.coffee' })
        atom.config.set('foo.bar.baz', 22, { scopeSelector: '.source .string.quoted.double' })
        atom.config.set('foo.bar.baz', 11, { scopeSelector: '.source' })
  ...
  describe('.unset(keyPath, {source, scopeSelector})', function () {
    ...
    describe("when no 'scopeSelector' is given", function () {
      describe("when a 'source' but no key-path is given", () =>
        it('removes all scoped settings with the given source', function () {
      ...
      describe("when a 'source' and a key-path is given", () =>
        it('removes all scoped settings with the given source and key-path', function () {
      ...
      return describe("when no 'source' is given", () =>
        it('removes all scoped and unscoped properties for that key-path', function () {
  ...
  describe('.onDidChange(keyPath, {scope})', function () {
    ...
    return describe("when a 'scope' is given", () =>
      it('calls the supplied callback when the value at the descriptor/keypath changes', function () {
  ...
  describe('.getSources()', () =>
    it("returns an array of all of the config's source names", function () {
  ...
  return describe('Internal Methods', function () {
    describe('.save()', function () {
      ...
      describe("when ~/.atom/config.json doesn't exist", () =>
        it('writes any non-default properties to ~/.atom/config.cson', function () {
          atom.config.set('a.b.c', 1)
          atom.config.set('a.b.d', 2)
          atom.config.set('x.y.z', 3)
          atom.config.setDefaults('a.b', { e: 4, f: 5 })
      ...
      describe('when scoped settings are defined', () =>
        it('writes out explicitly set config settings', function () {
...
`
      ...
      describe('when the config file does not exist', () =>
        it('creates it with an empty object', function () {
...
`
      ...
      describe('when there is a pending save', () =>
        it('does not change the config settings', function () {
...
`
      ...
      describe('when the config file changes to omit a setting with a default', () =>
        it('resets the setting back to the default', function () {
    ...
    describe('.initializeConfigDirectory()', function () {
      ...
      return describe("when the configDirPath doesn't exist", () =>
        it('copies the contents of dot-atom to ~/.atom', function () {
    ...
    describe('.unshiftAtKeyPath(keyPath, value)', () =>
      it('unshifts the given value to the array at the key path and updates observers', function () {
    ...
    describe('.removeAtKeyPath(keyPath, value)', () =>
      it('removes the given value from the array at the key path and updates observers', function () {
    ...
    return describe('.setSchema(keyPath, schema)', function () {
      ...
      describe('when the value has an "string" type', function () {
        ...
        return describe('when the schema has a "maximumLength" key', () =>
          it('trims the string to be no longer than the specified maximum', function () {
            const schema = {
              type: 'string',
              default: 'ok',
              maximumLength: 3
      ...
      describe('when the value has an "object" type', function () {
        ...
        describe('when the value has additionalProperties set to false', () =>
          it('does not allow other properties to be set on the object', function () {
            atom.config.setSchema('foo.bar', {
              type: 'object',
              properties: {
                anInt: {
        ...
        return describe('when the value has an additionalProperties schema', () =>
          it('validates properties of the object against that schema', function () {
            atom.config.setSchema('foo.bar', {
              type: 'object',
              properties: {
                anInt: {
